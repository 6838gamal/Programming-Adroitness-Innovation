...
import { Bot, Send, User, Loader2, Compass, Mic, StopCircle, Copy, Volume2, VolumeX, Speaker } from "lucide-react";
import { aiGuidanceAssistant } from "@/ai/flows/ai-guidance-assistant";
...
export function ChatUI() {
  ...
  const handleSendMessage = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        const assistantResponse = await aiGuidanceAssistant({
          query: currentInput,
  ...
  const handleManualPlayAudio = (text: string) => {
    if (isPlayingAudio && audioRef.current) {
        stopCurrentAudio();
        return;
    }
    ...
    <div className={`flex h-full flex-col rounded-lg border bg-card ${language === 'ar' ? 'font-arabic' : 'font-body'}`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <header className="flex items-center justify-between p-4 border-b">
        <div className="flex items-center gap-2">
            <Compass className="h-8 w-8 hidden sm:block text-primary" />
            <div>
              <h3 className="font-headline text-lg font-semibold">{t('career_guide')}</h3>
              <p className="text-sm text-muted-foreground">{t('career_guide_subtitle')}</p>
            </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          {messages.length === 0 && (
             <div className="flex flex-col items-center justify-center h-full pt-10 text-center">
              ...
              <h3 className="font-headline text-2xl font-semibold mt-4">{t('career_guide')}</h3>
              <p className="text-muted-foreground mt-2">{t('career_guide_subtitle')}</p>
            </div>
          )}
      ...
      <div className="border-t bg-card p-4">
        <form onSubmit={handleSendMessage} className="flex items-center gap-4">
          <Input
            ...
            placeholder={t('career_guide_placeholder')}
            className="flex-1"
